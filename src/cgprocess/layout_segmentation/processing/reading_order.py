...
def get_region_properties(bbox_dict: Dict[int, List[List[float]]]) -> ndarray:
    ...
    for label, bbox_list in bbox_dict.items():
        ...
        label_properties = np.vstack(
            [
                np.arange(len(bbox_ndarray)) + index,
                np.full(len(bbox_ndarray), label),
                bbox_ndarray[:, 0],
                bbox_ndarray[:, 2],
                np.mean([bbox_ndarray[:, 0], bbox_ndarray[:, 2]], axis=0),
                np.mean([bbox_ndarray[:, 1], bbox_ndarray[:, 3]], axis=0),
                bbox_ndarray[:, 2] - bbox_ndarray[:, 0],
                bbox_ndarray[:, 3] - bbox_ndarray[:, 1],
            ]
        ).T
...
def median(properties: ndarray) -> int:
    ...
    if len(text_regions) == 0:
        warnings.warn(
            "Reading Order has not found any paragraphs in this Page. "
...
def get_global_splitting_regions(
    properties: ndarray, x_median: int, columns_per_page: int
) -> ndarray:
    """
    ...
    """
    class_indices, rounded_width = get_width_in_terms_of_column(
        properties, x_median, [7]
    )
...
def get_local_splitting_regions(
    properties: ndarray, x_median: int, columns_per_page: int
) -> ndarray:
    """
    Determine divider indices. A region is considered a page divider, if it has the right class and if it
...
def get_width_in_terms_of_column(
    properties: ndarray, x_median: int, region_classes: List[int]
    ...
    rounded_width: ndarray = np.round(
        (properties[class_indices][:, 3] - properties[class_indices][:, 2]) / x_median
    )
...
def sort_column(column: ndarray) -> Tuple[ndarray, ...]:
    ...
    is_vertical = column[is_separator][:, 6] < column[is_separator][:, 7]
    ...
    is_border_separator = is_separator[
        is_right_border_separator + is_left_border_separator
    ]
    ...
    sorted_left_border_separator = left_border_to_sort[
        np.argsort(left_border_to_sort[:, 4])
    ]
    sorted_right_border_separator = right_border_to_sort[
        np.argsort(right_border_to_sort[:, 4])
    ]
...
class PageProperties:
    ...
    def __init__(
        self,
        bbox_dict: Dict[int, List[List[float]]],
        region_properties: Union[None, ndarray] = None,
    ):
        if region_properties is not None:
            assert (
                region_properties.shape[1] == 8
            ), f"Properties ndarray has to have the shape nx8. Found {region_properties.shape}"
        ...
        if len(self.region_properties) > 0:
            ...
            self.columns_per_page: int = round(
                (self.global_x_max - self.global_x_min) / self.x_median
            )

            self.global_splitting_bool = get_global_splitting_regions(
                self.region_properties, self.x_median, self.columns_per_page
            )
            self.local_splitting_bool = get_local_splitting_regions(
                self.region_properties, self.x_median, self.columns_per_page
            )
    ...
    def get_reading_order(self) -> Dict[int, int]:
        ...
        if len(self.region_properties) < 1:
            return {}
        self.global_divider_split(
            self.region_properties,
            self.global_splitting_bool,
            self.local_splitting_bool,
            result,
        )

        return {int(k): v for v, k in enumerate(result)}

    def global_divider_split(
        self,
        properties: ndarray,
        global_splitting_bool: ndarray,
        ...
        if any(global_splitting_bool):
            self.execute_single_global_split(
                properties, global_splitting_bool, local_splitting_indices, result
            )
        else:
            self.local_divider_split(properties, local_splitting_indices, result)

    def execute_single_global_split(
        self,
        properties: ndarray,
        global_splitting_bool: ndarray,
        local_splitting_bool: ndarray,
        result: List[int],
    ) -> None:
        """
        ...
        :param result: reading order result which is updated by reference
        """
        split_index = np.where(global_splitting_bool)[0][
            np.argmax(properties[global_splitting_bool][:, 5])
        ]
        ...
        self.global_divider_split(
            properties[np.invert(region_bool)],
            global_splitting_bool[np.invert(region_bool)],
            local_splitting_bool[np.invert(region_bool)],
            result,
        )
        result.append(divider_entry[0])  # type: ignore
        self.global_divider_split(
            properties[region_bool],
            global_splitting_bool[region_bool],
            local_splitting_bool[region_bool],
            result,
        )

    def local_divider_split(
        self, properties: ndarray, local_splitting_bool: ndarray, result: List[int]
    ) -> None:
        """
    ...
    def execute_single_local_split(
        self, properties: ndarray, local_splitting_bool: ndarray, result: List[int]
    ) -> None:
        """
        ...
        :param result: reading order result which is updated by reference
        """
        split_index = np.where(local_splitting_bool)[0][
            np.argmin(properties[local_splitting_bool][:, 5])
        ]
        divider_entry = properties[split_index]
        ...
        if sum(left_region_bool) > 0 and sum(right_region_bool) > 0:
            region_bool = (
                np.invert(top_region_bool) + left_region_bool + right_region_bool
            )
        else:
            region_bool = top_region_bool + left_region_bool + right_region_bool

        self.local_divider_split(
            properties[region_bool], local_splitting_bool[region_bool], result
        )
        result.append(divider_entry[0])  # type: ignore
        self.local_divider_split(
            properties[np.invert(region_bool)],
    ...
    def column_split(self, properties: ndarray, result: List[int]) -> None:
        ...
        while True:
            in_column: ndarray = (
                columns[:, 2] < (estimated_column_border - self.x_median / 2)
            ...
            (
                sorted_column,
                sorted_left_border_separator,
                sorted_right_border_separator,
            ) = sort_column(columns[in_column])
            ...
            actual_column_border = np.mean(
                sorted_column[np.isin(sorted_column[:, 1], [2, 3, 4, 5])][:, 3]
            )
            estimated_column_border = (
                actual_column_border + self.x_median
                if not np.isnan(actual_column_border)
                else estimated_column_border + self.x_median
            )
