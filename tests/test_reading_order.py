...
from src.cgprocess.layout_segmentation.processing.reading_order import (
    PageProperties,
    get_global_splitting_regions,
...
class TestClassExport:
    ...
    def test_splitting_regions(self) -> None:
        ...
        global_ground_truth = np.array(
            [1, 8]
        )  # actual indices are 2,9 but list begins with 0
        local_ground_truth = np.array([7])
        x_median = 25
        columns_per_page = 4

        result = np.where(
            get_global_splitting_regions(properties, x_median, columns_per_page)
        )[0].tolist()
        assert len(result) == len(global_ground_truth)
        assert all(result == global_ground_truth)

        result = np.where(
            get_local_splitting_regions(properties, x_median, columns_per_page)
        )[0].tolist()
